$(document).ready(() => {
    $.get(`${API_URL_BASE}/Engineer/PropertyAttachments/${FINCA_ID}`, (data) => {
    ...
    $("#btnCargarApis").on("click", () => {
        ...
        $.get(`${API_URL_BASE}/Engineer/EvaluationApiData/${FINCA_ID}`, (res) => {
    ...
    function enviarDictamen(tipo) {
        ...
        Swal.fire({
            ...
            $.ajax({
                url:         `${API_URL_BASE}/Engineer/Verdict/${FINCA_ID}`,
                method:      "POST",
                    ...
                    if (res.success) {
                        if (tipo === "Aprobar") {
                            ...
                            Swal.fire({ icon: "success", title: "Dictamen aplicado", text: res.message })
                                .then(() => window.location.href = "/Engineer/FifoQueue");
                        }
                    } else {
    ...
    $("#btnActivarPlan").on("click", () => {
        Swal.fire({
            ...
            $.ajax({
                url:    `${API_URL_BASE}/Engineer/ActivatePlan/${FINCA_ID}`,
                method: "POST",
                success: (res) => {
