...
function cargarUsuarios() {
    $.get(`${API_URL_BASE}/Admin/Usuarios/Data`, data => gridOptions.api?.setRowData(data))
...
function confirmarInactivar(id, nombre) {
    Swal.fire({
        ...
        $.post(`${API_URL_BASE}/Admin/Usuarios/Inactivar/${id}`)
            .done(res => {
...
function confirmarReactivar(id, nombre) {
    Swal.fire({
        ...
        if (!r.isConfirmed) return;
        $.post(`${API_URL_BASE}/Admin/Usuarios/Reactivar/${id}`)
            .done(res => {
                if (res.success) { Swal.fire({ icon: "success", text: res.message }); cargarUsuarios(); }
                else showAlert("alertUsuarios", res.message, "danger");
            });
...
$(document).ready(() => {
    ...
    $("#btnGuardarIngeniero").on("click", () => {
        ...
        $.ajax({
            url: `${API_URL_BASE}/Admin/Usuarios/Crear`, method: "POST",
    ...
    $("#btnGuardarEditar").on("click", () => {
        ...
        $.ajax({
            url: `${API_URL_BASE}/Admin/Usuarios/Editar/${id}`, method: "POST",
            contentType: "application/json", data: JSON.stringify(payload),
            success: (res) => {
